...
var mushroom_preload = preload("res://scene/Mobs/evil_mushroom.tscn")
...
## Эта функция самописная, хоть _ и заставляет думать иначе. Дело в том, что префиксом тут является вся _on. Он означает, что эта функция предназначена для обработки сигнала
## Собственно я и забыл о ней потому, что написал её сам, а она вызывается привязкой узла "таймаут"
...
func mushroom_spawn():
	...
	mushroom.position = Vector2(randi_range(-500, -100), 640)
