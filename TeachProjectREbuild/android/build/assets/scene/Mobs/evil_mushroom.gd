...
## Переформатировали код по "композиции" Более тут нет здоровья и всё, что его касается, находится в скриптах сцены mob_health, а оттуда приходит в виде сигналов сюда. Сигналы
## кастомные и появились, так как мы сюда привязали через цепи слева сверху MobHealth
